...
library(patchwork)        # For vertical stacking of plots (plot rendering)
...
```{r}
# Step 6: Load Ortholog Mapping File from gprofiler2    #15352
orthologs <- read_csv("~/human_to_orthologs.csv")
#View(orthologs)

# Assess Gene Counts Across Datasets: Total genes in each dataset
n_genes_gene_list <- length(unique(full_gene_list$Gene))
n_genes_orthologs <- length(unique(orthologs$ortholog_name))

# Number of shared genes = 13431
shared_genes <- intersect(full_gene_list$Gene, orthologs$ortholog_name)
n_shared_genes <- length(shared_genes)

# Report Summary Statistics
cat("Mouse gene list contains:", n_genes_gene_list, "genes\n")                            #= 22899
cat("Orthologs file contains:", n_genes_orthologs, "mouse genes via human orthologs\n")   #= 15352
cat("Number of genes shared between the two:", n_shared_genes, "\n")                     #= 13431
```


```{r}
# Step 7: Map Mouse Genes to Human Orthologs
...
```{r}
# Step 8: Load the phylostrata file you uploaded earlier
phylostrata <- read_csv("~/gene_phylostrata.csv")
#View(phylostrata)
...
```{r}
#STEP 9: Add Phylostrata Annotations to Seurat Object (Part A: Gene Symbols / non-RIK IDs)
...
Summary:
This step is a pragmatic solution to handle duplicates to allow for unique rownames (required by Seurat). 
However, it sacrifices the richness of duplicated data and assumes all duplicates are equivalent, which might not always hold biologically. 

	
```{r}
#STEP 10: Add Phylostrata to Seurat Object Metadata: part b (RidIDs)
...
```{r}
#Step 11: Combining Gene Sets and Merging Phylostrata Metadata Across Seurat Subsets
...
# Step 1: Extract phylostrata metadata safely
phylostrata_df <- combined_seurat[["RNA"]]@misc$Phylostrata %>%
  as.data.frame() %>%
  rownames_to_column("Gene")

# Step 2: Ensure the Phylostrata column is a clean character vector, then numeric
phylostrata_df$Phylostrata <- as.character(phylostrata_df$Phylostrata)
phylostrata_df <- phylostrata_df %>%
  filter(!is.na(Phylostrata)) %>%
  mutate(Phylostrata = as.numeric(Phylostrata))   # Now numeric

# Step 3: Count genes per phylostratum
...
# Step 4: Plot
ggplot(phylostrata_counts, aes(x = factor(Phylostrata), y = NumGenes, fill = Phylostrata)) +
  geom_bar(stat = "identity", colour = "black", width = 0.8) +
  geom_text(aes(label = NumGenes), vjust = -0.4, size = 3.5) +
  scale_fill_gradient(low = "#4CBB17", high = "#FF55A3") +
  labs(
    title = "Gene Distribution Across Phylostrata",
    subtitle = paste("Total mapped genes:", nrow(phylostrata_df)),
    x = "Phylostratum",
    y = "Number of Genes"
  ) +
...
for (tis in names(de_results_list)) {
  ...
  up_in_3m <- rownames(de_res[de_res$avg_log2FC > 0 & de_res$p_val_adj < 0.05, ])
  up_in_24m <- rownames(de_res[de_res$avg_log2FC < 0 & de_res$p_val_adj < 0.05, ])
...
# Load libraries
library(ggplot2)
library(patchwork)  # for safe panel layout

# Step 1: Select tissues to panel
panel_tissues <- c("Large_Intestine", "Thymus", "Spleen")

# Step 2: Extract the plots from volcano_list
panel_plots <- volcano_list[panel_tissues]

# Step 3: Combine the plots using patchwork
final_plot <- panel_plots[[1]] + panel_plots[[2]] + panel_plots[[3]] +
  plot_layout(ncol = 3) +
  plot_annotation(tag_levels = "A")  # Adds A, B, C tags

# Step 4: Print to viewer (or save)
print(final_plot)

# Optional: Save to file
ggsave("volcano_panel_plot.pdf", final_plot, width = 15, height = 5)
```
...
Part 4: GO Enrichment After Tissue-Wise DE Analysis
```{r}
...
```{r}
#Identification of ALL Significant Age-Associated DEGs Annotated by Evolutionary Origin

# Load DE results and phylostrata
de_results_list <- readRDS("de_results_list.rds")
phylostrata_df <- as.data.frame(combined_seurat[["RNA"]]@misc$Phylostrata)
phylostrata_df$Gene <- rownames(phylostrata_df)

# Initialise list
combined_sig_de_list <- list()
...
for (tis in names(de_results_list)) {
  ...
  de_res_sig <- de_res_sig %>%
    mutate(direction = ifelse(avg_log2FC > 0, "Up in 3m", "Up in 24m"))
  
  # Merge phylostrata
  de_merged <- left_join(de_res_sig, phylostrata_df, by = "Gene")
  colnames(de_merged)[colnames(de_merged) == "Phylostrata"] <- "phylostrata"
  
  # Add to list
  combined_sig_de_list[[tis]] <- de_merged
}

# Combine all into one dataframe
combined_sig_de_all <- bind_rows(combined_sig_de_list)

# View top rows
head(combined_sig_de_all)

# Save to CSV
write.csv(combined_sig_de_all, "significant_DE_genes_all_tissues.csv", row.names = FALSE)
```
...
# Prepare phylostrata dataframe
phylostrata_raw <- combined_seurat[["RNA"]]@misc$Phylostrata
phylostrata_df <- if (is.null(dim(phylostrata_raw))) {
  data.frame(gene = names(phylostrata_raw),
             phylostratum = as.integer(phylostrata_raw),
             stringsAsFactors = FALSE)
} else {
  data.frame(gene = rownames(phylostrata_raw),
             phylostratum = as.integer(phylostrata_raw[, 1]),
             stringsAsFactors = FALSE)
}
...
for (tis in names(de_results_list)) {
  ...
  # Get genes upregulated in 24m
  genes_up_24m <- rownames(de_res %>% filter(avg_log2FC < 0, p_val_adj < 0.05))
  if (length(genes_up_24m) < 5) {
    cat("Too few genes for GO enrichment in", tis, "- skipping\n")
    next
  }
  ...
  entrez_genes <- map_and_filter_genes(genes_up_24m)
  if (length(entrez_genes) < 5) {
    cat("Too few mapped Entrez IDs for", tis, "- skipping\n")
    next
  }
  ...
  if (is.null(ego_bp) || nrow(ego_bp) == 0) {
    cat("No significant GO terms for", tis, "\n")
    next
  }
  
  # Parse geneID field (symbols, already readable)
  gene2go <- setNames(ego_bp@result$geneID, ego_bp@result$ID)
  
  go_gene_df <- tibble(
    GO = rep(names(gene2go), lengths(strsplit(gene2go, "/"))),
    Symbol = unlist(strsplit(gene2go, "/"))
  )
  
  # Join phylostrata annotation by SYMBOL
  go_gene_df <- go_gene_df %>%
    left_join(phylostrata_df, by = c("Symbol" = "gene"))
  
  # Summarise gene count per GO term and phylostratum
  go_phylostratum_counts <- go_gene_df %>%
    group_by(GO, phylostratum) %>%
    summarise(count = n(), .groups = "drop")
  
  # Select top 10 GO terms for display
  top_go_terms <- ego_bp@result %>%
    arrange(p.adjust) %>%
    slice_head(n = 10) %>%
    pull(ID)
  
  plot_data <- go_phylostratum_counts %>%
    filter(GO %in% top_go_terms) %>%
    mutate(GO = factor(GO, levels = rev(top_go_terms)))
  
  # --- BARPLOT ---
  p_bar <- ggplot(plot_data, aes(x = GO, y = count, fill = factor(phylostratum))) +
    geom_col(position = position_stack()) +
    coord_flip() +
    labs(
      title = paste("GO Biological Process Enrichment by Phylostrata in", tis),
      x = "GO Term",
      y = "Number of Genes",
      fill = "Phylostratum"
    ) +
  ...
  # --- DOTPLOT ---
  gene_counts_per_go <- dplyr::filter(ego_bp@result, ID %in% top_go_terms) %>%
  dplyr::select(ID, GeneRatio)
  
  gene_ratios <- sapply(gene_counts_per_go$GeneRatio, function(x) eval(parse(text = x)))
  
  plot_data <- plot_data %>%
    left_join(gene_counts_per_go, by = c("GO" = "ID")) %>%
    mutate(GeneRatioNum = gene_ratios[match(GO, gene_counts_per_go$ID)],
           Ratio = count / (GeneRatioNum * 100))  # crude scaling
  
  plot_title <- paste("GO Enrichment Dotplot by Phylostrata in", tis)
  
  p_dot <- ggplot(plot_data, aes(x = GO, y = factor(phylostratum), size = count, color = Ratio)) +
    geom_point() +
